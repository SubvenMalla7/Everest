...
namespace Everest_Video_Library.Controllers.VideoLibrary
{
    ...
    public class MembersController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                member.Loans = db.Loans.Where(X => X.MemberId == member.Id);
            }
        ...
        public ActionResult Details(int? id)
        {
            ...
            var lones = db.Loans.Where(X => X.MemberId == id && DateTime.Compare(X.LoanDate,DateTime.Today)<=31).ToList();
        ...
        {
            ViewBag.CatagoryId = new SelectList(db.MemberCategories, "Id", "Name");
            return View();
        }
        ...
        public ActionResult Create([Bind(Include = "Id,CatagoryId,DateOfBirth,FirstName,LastName,Email,Address,Gender")] Member member)
        {
            ...
            ViewBag.CatagoryId = new SelectList(db.MemberCategories, "Id", "Name", member.CatagoryId);
            return View(member);
        }
        ...
        public ActionResult NotBorrowed()
        {
            ...
            foreach(Member member in members.ToList())
            {
                ...
                var lones = db.Loans.Where(X => X.MemberId == member.Id && DateTime.Compare(before,X.LoanDate)<0).ToList();
                if(lones.Count != 0)
                {
